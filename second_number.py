...
class SecondNumber(Node):
    def __init__(self, node_name, *, context = None, cli_args = None, namespace = None, use_global_arguments = True, enable_rosout = True, start_parameter_services = True, parameter_overrides = None, allow_undeclared_parameters = False, automatically_declare_parameters_from_overrides = False, enable_logger_service = False):
        ...
        self.publisher_ = self.create_publisher(Int64, "number2_topic", 10)
        self.get_logger().info("Second number generator has been started now we will send the first number")
         

    def publish_number(self):
        number = int(input("enter the number2:"))
        msg = Int64()
        msg.data = number
...
def main(args=None):
    ...
    node = SecondNumber("nodee_second_number")
    node.publish_number() 
    node.destroy_node()
    rclpy.shutdown()
